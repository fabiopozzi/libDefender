...
uint _libDefender_stackVariableP(void *addr) {
    ...
    /* siamo dentro a questa funzione, chiamata dal wrapper, chiamato dalla funzione a rischio */
    /* dobbiamo quindi risalire di 3 frame */
	unw_step(&cursor);
	unw_step(&cursor);
	unw_step(&cursor);
	unw_get_reg(&cursor, UNW_REG_SP, &sp);
...
int _libDefender_save_ra_fp(int maxcount, caddr_t *ra_array, caddr_t *fp_array) {
	...
	// return an array with return addresses and frame pointer (if any)
	while (unw_step(&cursor)>0)
	{
		 ...
		 * address will be the word immediately before the caller top of the stack pointer.
		 */
		*ra_p++ = *(caddr_t*)(sp- size); //store the return address into the ra_p array
		count++;
	}

	//condizione di uscita dal ciclo giusta??
	return count;
